...
public class CommonUserConverter implements UserAuthenticationConverter
{
    ...
    public Authentication extractAuthentication(Map<String, ?> map)
    {
        if (map.containsKey(USERNAME))
        {
            ...
            Long merchantId = Convert.toLong(map.get(SecurityConstants.DETAILS_M_ID));
            String username = (String) map.get(SecurityConstants.DETAILS_USERNAME);
            LoginUser user = new LoginUser(userId,merchantId, username, N_A, true, true, true, true, authorities);
